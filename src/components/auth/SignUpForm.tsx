...
export default function SignUpForm() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    // Placeholder submission — swap for a real account-creation call once auth is connected.
  ...
  if (status === "success") {
    return (
      <AuthStatusNotice
        ...
        description="This is a frontend preview — Democratic Pulse doesn't create accounts yet. Real authentication will be connected here."
        onReset={reset}
      />
    );
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <AuthForm onSubmit={handleSubmit} status={status} className="flex flex-col gap-5">
        ...
        <AuthSubmitButton loading={status === "submitting"}>Create account</AuthSubmitButton>
      </AuthForm>
    </div>
  );
}
